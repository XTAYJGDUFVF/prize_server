...
class InitialInfo(RequestBaseHandler):
    ...
    def get(self):
        ...
        if not initial_chance[0]:
            ...
            return self.write_json(ErrorCode.get_error_response(error_code), 400)
        return self.write_json({r'left_chance': initial_chance[1][0],'used_card':initial_chance[1][1], r'prize_info': prize_info[1]}, 200)
...
class GetPrize(RequestBaseHandler):
    ...
    def get(self):
        ...
        return self.write_json({'prize_id': prize_id,
                                'left_chance': left_chance - 1,
                                'prize_name': prize_need_list[prize_id - 1][prize_id]['prize_name'],
                                'notice_statu': prize_need_list[prize_id - 1][prize_id]['notice_statu']},
                                200)
    ...
    def get_prize_id(self, uid, prize_need_list):
        ...
        if prize_need_list[prize_id - 1][prize_id]['prize_num'] == None:
            return prize_id
        m_prize_log = PrizeLog()
        per_today_prize_count = yield m_prize_log.per_today_prize_count(uid, prize_id)  # 今天该人获得该奖项的总数
        ...
        if all_today_prize_count < largest_perday and per_today_prize_count < largest_per_get:
            return prize_id
        if all_today_prize_count >= largest_perday or per_today_prize_count >= largest_per_get:
            prize_last_id = yield self.get_prize_id(uid, prize_need_list)
            return prize_last_id
